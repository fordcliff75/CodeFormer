...
def get_center_face(det_faces, h=0, w=0, center=None):
    ...
    # Calculate pure-Python squared Euclidean distance to avoid np.array allocation
    # and np.linalg.norm overhead inside the loop
    center_dist = [
        (((det_face[0] + det_face[2]) / 2) - cx) ** 2 +
        (((det_face[1] + det_face[3]) / 2) - cy) ** 2
