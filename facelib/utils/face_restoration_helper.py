...
def get_center_face(det_faces, h=0, w=0, center=None):
    ...
    center_dist = []
    for det_face in det_faces:
        face_center = np.array([(det_face[0] + det_face[2]) / 2,
                                (det_face[1] + det_face[3]) / 2])
    ...
    def paste_faces_to_input_image(self,
        ...
        for restored_face, inverse_affine in zip(self.restored_faces,
            ...
            if self.use_parse:
                ...
                # Replace iterative loop that maps face parsing indices to a colormap with NumPy advanced indexing
                # This reduces time complexity from O(N_classes * Image_Size) to O(Image_Size)
