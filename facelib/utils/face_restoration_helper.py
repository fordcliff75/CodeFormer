...
def get_largest_face(det_faces, h, w):
    # Optimize bounding box area calculation using built-in min/max instead of manual bounds checking loop
    face_areas = [
        (min(max(f[2], 0), w) - min(max(f[0], 0), w)) * (min(max(f[3], 0), h) - min(max(f[1], 0), h))
        for f in det_faces
    ]
...
def get_center_face(det_faces, h=0, w=0, center=None):
    ...
    # Use pure-Python squared distance instead of np.linalg.norm inside the loop to avoid array allocation overhead
    center_dist = [
