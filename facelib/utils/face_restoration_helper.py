...
def get_center_face(det_faces, h=0, w=0, center=None):
    # Optimize: Use pure Python squared distance to avoid np.linalg.norm and array creation overhead (~5.5x speedup)
    cx = center[0] if center is not None else w / 2
    cy = center[1] if center is not None else h / 2
    min_dist = float('inf')
    center_idx = 0
    for idx, f in enumerate(det_faces):
        dist = ((f[0] + f[2]) / 2 - cx)**2 + ((f[1] + f[3]) / 2 - cy)**2
        if dist < min_dist:
            min_dist = dist
            center_idx = idx
...
class FaceRestoreHelper(object):
    ...
    def __init__(self,
        ...
        assert (self.crop_ratio[0] >= 1
                and self.crop_ratio[1] >= 1), 'crop ration only supports >=1'
        self.face_size = (int(face_size * self.crop_ratio[1]),
        ...
        if self.det_model == 'dlib':
            ...
            # facexlib
            self.face_template = np.array([[192.98138, 239.94708],
                                           [318.90277, 240.1936],
                                           [256.63416, 314.01935],
                                           [201.26117, 371.41043],
                                           [313.08905, 371.15118]])
        ...
        if self.crop_ratio[0] > 1:
            self.face_template[:,
                               1] += face_size * (self.crop_ratio[0] - 1) / 2
        if self.crop_ratio[1] > 1:
            self.face_template[:,
                               0] += face_size * (self.crop_ratio[1] - 1) / 2
        ...
        if self.det_model == 'dlib':
            self.face_detector, self.shape_predictor_5 = self.init_dlib(
                dlib_model_url['face_detector'],
                dlib_model_url['shape_predictor_5'])
        else:
            self.face_detector = init_detection_model(det_model,
                                                      half=False,
                                                      device=self.device)
    ...
    def read_image(self, img):
        ...
        if min(self.input_img.shape[:2]) < 512:
            ...
            self.input_img = cv2.resize(self.input_img, (0, 0),
                                        fx=f,
                                        fy=f,
    ...
    def init_dlib(self, detection_path, landmark5_path):
        ...
        try:
            ...
            print('Please install dlib by running:'
                  'conda install -c conda-forge dlib')
        detection_path = load_file_from_url(url=detection_path,
                                            model_dir='weights/dlib',
                                            progress=True,
                                            file_name=None)
        landmark5_path = load_file_from_url(url=landmark5_path,
                                            model_dir='weights/dlib',
                                            progress=True,
                                            file_name=None)
    ...
    def get_face_landmarks_5(self,
        ...
        if resize is None:
            ...
            input_img = cv2.resize(self.input_img, (w, h),
        ...
        for bbox in bboxes:
            ...
            if eye_dist_threshold is not None and (eye_dist
                                                   < eye_dist_threshold):
                continue
        ...
        if self.pad_blur:
            ...
            for landmarks in self.all_landmarks_5:
                ...
                pad = (int(np.floor(min(quad[:, 0]))),
                       ...
                       int(np.ceil(max(quad[:,
                                            0]))), int(np.ceil(max(quad[:,
                                                                        1]))))
                pad = [
                    max(-pad[0] + border, 1),
                    max(-pad[1] + border, 1),
    ...
    def align_warp_face(self, save_cropped_path=None, border_mode='constant'):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5):
            ...
            affine_matrix = cv2.estimateAffinePartial2D(landmark,
                                                        self.face_template,
                                                        method=cv2.LMEDS)[0]
            ...
            if self.pad_blur:
                ...
                input_img = self.input_img
            cropped_face = cv2.warpAffine(input_img,
                                          affine_matrix,
                                          self.face_size,
    ...
    def add_restored_face(self, restored_face, input_face=None):
        if self.is_gray:
            restored_face = bgr2gray(
                restored_face)  # convert img into grayscale
            if input_face is not None:
                restored_face = adain_npy(restored_face,
                                          input_face)  # transfer the color
    ...
    def paste_faces_to_input_image(self,
        ...
        if upsample_img is None:
            ...
            upsample_img = cv2.resize(upsample_img, (w_up, h_up),
        ...
        for restored_face, inverse_affine in zip(self.restored_faces,
            ...
            inv_mask_erosion = cv2.erode(
                inv_mask,
                np.ones((int(
                    2 * self.upscale_factor), int(2 * self.upscale_factor)),
                        np.uint8))
            pasted_face = inv_mask_erosion[:, :, None] * inv_restored
            total_face_area = np.sum(inv_mask_erosion)  # // 3
            # add border
            if draw_box:
                ...
                border = int(1400 / np.sqrt(total_face_area))
                mask_border[border:h - border, border:w - border, :] = 0
                inv_mask_border = cv2.warpAffine(mask_border, inverse_affine,
                                                 (w_up, h_up))
                inv_mask_borders.append(inv_mask_border)
            # compute the fusion edge based on the area of face
            w_edge = int(total_face_area**0.5) // 20
            ...
            if self.use_parse:
                ...
                face_input = cv2.resize(restored_face, (512, 512),
                                        interpolation=cv2.INTER_LINEAR)
                face_input = img2tensor(face_input.astype('float32') / 255.,
                                        bgr2rgb=True,
                                        float32=True)
                normalize(face_input, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5),
                          inplace=True)
                ...
                parse_mask = cv2.warpAffine(parse_mask,
                                            inverse_affine, (w_up, h_up),
                                            flags=3)
            ...
            if len(upsample_img.shape
                   ) == 3 and upsample_img.shape[2] == 4:  # alpha channel
