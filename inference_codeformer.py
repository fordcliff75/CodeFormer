...
from basicsr.utils import imwrite, img2tensor, tensor2img_fast
...
def set_realesrgan():
    ...
    if torch.cuda.is_available():  # set False in CPU/MPS mode
        ...
        if not any(gpu in torch.cuda.get_device_name(0)
                   for gpu in no_half_gpu_list):
            use_half = True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--only_center_face',
                        action='store_true',
    ...
    parser.add_argument('--save_video_fps',
                        type=float,
                        default=None,
    ...
    if args.input_path.endswith(
        ...
        ('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')):  # input video path
        ...
        ) if args.save_video_fps is None else args.save_video_fps
        ...
        for idx, cropped_face in enumerate(face_helper.cropped_faces):
            ...
            try:
                with torch.no_grad():
                    ...
                    # Performance optimization: Replace `tensor2img` with `tensor2img_fast`.
                    # Why: Performs clamping, permuting, and normalization on the GPU before transferring to CPU,
                    # bypassing the slow CPU-bound loop and math operations in `tensor2img`.
                    # Impact: Provides significant speedup during heavy inference workloads without quality loss.
                    restored_face = tensor2img_fast(output,
                                                    rgb2bgr=True,
                                                    min_max=(-1, 1))
                del output
                ...
                restored_face = tensor2img_fast(cropped_face_t,
                                                rgb2bgr=True,
                                                min_max=(-1, 1))
    ...
    if input_video:
        ...
        video_frames = []
        img_list = sorted(
            glob.glob(os.path.join(result_root, 'final_results',
                                   '*.[jp][pn]g')))
